# Agent Note: 前端与表现层 v1——沉浸聊天式 UI + 剧本资产/主题系统 + 多剧本管理
...
- **视觉小说式主界面**：用户明确否决（沉浸聊天式）。
- **引擎跑在浏览器**：fs/YAML/API key 决定不可行；拒绝。
- **资产字段内联进 npc/location/item schema**：改动面 ×3、strict schema 连锁；拒绝，单一 `assets.yaml` 索引（每个事实一个家）。
- **脚本导入走 public/ 静态目录**：运行时导入的剧本无法覆盖静态目录；拒绝，Route Handler 流式服务 + 防穿越。
- **前端状态库（redux/zustand）或动画库**：v1 规模 Context+reducer + CSS transition 足够；拒绝新依赖。
- **存档 v2 迁移层**：与敏捷原则冲突；拒绝，v3 直接断。
