...
test("desktop chat shell keeps one axis for messages, media and composer", async ({ page }) => {
  ...
  const geometry = await page.evaluate(() => {
    ...
    const scroll = document.querySelector('.cg-transcript');
    ...
    return {
      ...
      transcript: rect('.cg-transcript'),
      inner: rect('.cg-transcript__inner'),
      composer: rect('.cg-composer'),
      composerGroup: rect('.cg-composer .cg-input-group'),
      world,
      worldBodies,
      playerBodies,
      media,
...
test("mobile chat shell keeps controls reachable without page overflow", async ({ page }) => {
  ...
  const geometry = await page.evaluate(() => {
    ...
    const transcript = document.querySelector(".cg-transcript");
    return {
      viewport: { width: innerWidth, height: innerHeight },
      body: { width: document.body.clientWidth, scrollWidth: document.body.scrollWidth, height: document.body.clientHeight, scrollHeight: document.body.scrollHeight },
      trigger: box(".cg-mobile-nav-trigger"),
      shell: box(".cg-game-shell"),
      transcript: box(".cg-transcript"),
