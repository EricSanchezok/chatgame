...
export async function verifyPostmortems(repoRoot) {
  ...
  const legacyNumbering = (manifest?.artifactPolicy?.legacy ?? []).some((entry) => entry.path.startsWith(`${dirRel}/`));
  const sequenceStart = legacyNumbering ? 1 : 0;
  for (let index = 0; index < numbers.length; index++) {
    if (numbers[index] !== sequenceStart + index) {
      errors.push(`postmortem numbers must be sequential starting at ${sequenceStart}; found ${files[index].name}`);
      break;
    }
  }
