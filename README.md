...
`test:live:qwen` 使用校园网 `Qwen3.8-27B` Profile，`test:live:glm` 使用 Blackmarsh 的默认 GLM Profile，`test:live:deepseek` 使用同一 smoke 实现和临时 DeepSeek Profile 映射；三条路径都必须形成 revision。Qwen 路径要求 `INF_API_KEY`，GLM 和 DeepSeek 分别要求 `ZHIPU_CODING_PLAN_API_KEY` 或 `DEEPSEEK_API_KEY`，不打印或持久化密钥。
