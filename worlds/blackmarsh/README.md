...
- [`world/`](world/)：可直接校验、打包和导入的 schema v11 世界目录。
...
应用创建全新的本地数据库时，会把 `world/` 内容打包为内存中的单一世界根目录 ZIP，并通过与用户世界相同的严格导入路径安装一次。已有数据库不会在启动时自动覆盖或恢复该世界；手工验证安装流程时，仍可打包 `world/` 并从世界库导入。
