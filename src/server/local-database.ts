...
export class LocalDatabase implements WorldRepository, WorldSessionStore, ExecutionLedger {
  ...
  private migrate(): void {
    this.connection.transaction(() => {
      ...
      if ((current.version ?? 0) > 2) throw new Error("local database schema is newer than this application");
      if ((current.version ?? 0) === 0) this.connection.exec(`
      ...
      if ((current.version ?? 0) < 2) {
        this.connection.exec(`
          ...
          CREATE INDEX executions_session_step ON executions(session_id, step, started_at);
          ...
          CREATE INDEX execution_events_session ON execution_events(
            json_extract(correlation_json, '$.sessionId'), sequence
          );
        `);
        this.connection.prepare("INSERT INTO schema_migrations(version, applied_at) VALUES (2, ?)")
          .run(new Date(this.now()).toISOString());
      }
    })();
  }
  ...
  beginExecution(input: BeginExecutionInput): ExecutionTraceWriter {
    ...
    this.connection.prepare(`
      INSERT INTO executions(
        id, kind, parent_execution_id, session_id, run_id, step, manifest_json,
  ...
  appendExecutionEvents(executionId: string, inputs: readonly RuntimeEventInput[]): RuntimeEvent[] {
    ...
    const events = this.connection.transaction(() => {
      this.assertInstanceLease();
  ...
  finishExecution(executionId: string, input: FinishExecutionInput): import("../engine/execution").ExecutionRef {
    ...
    const reference = this.connection.transaction(() => {
      ...
      const errorArtifactHash = input.error === undefined
        ? null
        : this.putExecutionArtifact(
            executionId,
            "execution.error",
            input.error instanceof Error ? serializeRuntimeError(input.error) : input.error,
          );
  ...
  sessionEvents(sessionId: string): RuntimeEvent[] {
    const runningIds = this.connection.prepare(
      "SELECT id FROM executions WHERE session_id = ? AND status = 'running'",
    ).all(sessionId) as Array<{ id: string }>;
    for (const { id } of runningIds) this.executionWriters.get(id)?.flush();
    return this.eventsFor("executions.session_id", sessionId);
  }
  ...
  createAndFinishExecution(
    ...
    correlation?: RuntimeCorrelation,
  ): { session: StoredWorldSession; executionRef: import("../engine/execution").ExecutionRef } {
    this.executionWriters.get(executionId)?.flush();
    return this.connection.transaction(() => {
      ...
      const writeStartedAt = performance.now();
      const session = this.create(finalizedDocument, correlation);
  ...
  compareAndSwapAndFinishExecution(
    ...
    return this.connection.transaction(() => {
      ...
      const expectedRef = this.currentExecutionRef(executionId);
      const finalizedDocument = structuredClone(document);
      finalizedDocument.state = attachExecutionRef(finalizedDocument.state, expectedRef, "step");
      const documentUtf8Bytes = Buffer.byteLength(JSON.stringify(canonicalize(finalizedDocument)), "utf8");
      const writeStartedAt = performance.now();
      const session = this.compareAndSwap(
        sessionId,
        expectedGeneration,
        finalizedDocument,
        correlation,
