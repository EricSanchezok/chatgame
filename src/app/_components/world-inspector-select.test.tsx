...
describe("WorldInspectorSelect", () => {
  ...
  it("renders the product-styled listbox instead of a native select popup", () => {
    ...
    const trigger = screen.getByRole("button", { name: "排序：引擎顺序 · 最新在上" });
    expect(trigger).toHaveAttribute("aria-haspopup", "listbox");
    expect(screen.queryByRole("listbox")).not.toBeInTheDocument();

    fireEvent.click(trigger);
    expect(screen.getByRole("listbox", { name: "排序" })).toBeVisible();
    expect(screen.getByRole("option", { name: "引擎顺序 · 最新在上" })).toHaveAttribute("aria-selected", "true");
  });
  ...
  it("supports arrow navigation, selection and Escape focus restoration", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "排序：引擎顺序 · 最新在上" }));
    fireEvent.keyDown(screen.getByRole("listbox"), { key: "Escape" });
    expect(screen.queryByRole("listbox")).not.toBeInTheDocument();
    expect(document.activeElement).toBe(screen.getByRole("button", { name: "排序：引擎顺序 · 最新在上" }));
  });
});
