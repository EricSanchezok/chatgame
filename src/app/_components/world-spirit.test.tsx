...
describe("world spirit", () => {
  ...
  it("derives a stable visual identity from the world content hash", () => {
    const renderSpirit = (worldContentHash: string) => {
      const view = render(
        <WorldSpirit
          appReducedMotion
          gaze={null}
      ...
      const markup = view.container.querySelector("svg")?.outerHTML;
      view.unmount();
      return markup;
    };

    expect(renderSpirit("sha256:world-one")).toBe(renderSpirit("sha256:world-one"));
    expect(renderSpirit("sha256:world-one")).not.toBe(renderSpirit("sha256:world-two"));
  });
