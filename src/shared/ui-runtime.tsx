...
export function ActionChoice({
  ...
  className,
  ...props
}: Omit<ComponentProps<"button">, "aria-pressed"> & {
  selected?: boolean;
  detail?: ReactNode;
}) {
  return (
    <button
      ...
      data-slot="action-choice"
      className={cn("cg-action-choice", className)}
      {...props}
    >
      <span>{children}</span>
...
export function Metric({
  ...
  className,
  ...props
}: ComponentProps<"div"> & { label: ReactNode; value: ReactNode; hint?: ReactNode }) {
  return (
    <div data-slot="metric" className={cn("cg-metric", className)} {...props}>
      <span>{label}</span>
      <strong>{value}</strong>
