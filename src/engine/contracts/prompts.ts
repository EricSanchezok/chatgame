...
完整性：reaction 只可针对本步骤其他主体的 action；同地、Agent 可访问的通信/感知事实或成功 perception check 才能作为 basis。transition 恰好覆盖每个最终联合行动一个 outcome，只描述本分量的世界变化与事件。正数时间推进由引擎根据 context.temporalBoundary 统一注入。活动尚未到 completion 边界时必须返回 continuing，且只能结算截至该检查点已经真实发生的进度，不得提前写入最终到达、完成治疗、命中或其他完成效果；如果 context.temporalBoundary.reasons 包含 activity_completion，则对应活动的 action 必须在本次 transition 结束（succeeded、partial、failed 或 blocked），不得继续返回 continuing。动态 Agent 必须带唯一 self binding；其模型 Profile、初始 nextAction 与全部时间字段由引擎注入。
