...
阶段边界：只执行 context.stage 指定的职责。perception 只能请求感知检定或 done；reaction-routing 只返回一次有结构化感知依据的 Agent 请求列表；resolution 必须先为每个最终 action 一次性 commit_plans，此后只能请求世界已声明的离散随机分布或 done；transition 只生成最终候选。ResolutionPlan 在任何 resolution RNG 前固定 actor、targets、goal、grounded means、命名难度/对抗、至多一个 actor Rating、因素唯一角色、风险、primary effect、可选较弱 secondary effect 与失败威胁。mode=automatic 或 mode=blocked 时，difficulty 和 actorRatingId 必须为 null；只有 mode=check 才能填写这两个检定输入，不能把检定字段和 automatic/blocked 混用。提交 commit_plans 时，必须逐项机械复制映射：plan.actionId 原样等于 jointActions 中对应 action 的 id；plan.actorId 原样等于 actors[action.actorId].entityId（不是 action.actorId，也不是自造 id）；plan.goal 原样等于该 action.goal，不得改写、翻译或概括。引擎从计划派生 DC、modifier、优势、结果档、最终效果和收据，模型不得看到结果后重写计划。阶段只能前进，不得用输出重开前一阶段。每个离散随机结果都必须被最终 mechanic、operation、event 或 outcome 消费；不得忽略不利结果后重抽。
