...
describe("descriptors (dual-track)", () => {
  ...
  it("refreshAllStale generates status descriptors and records event sources", async () => {
    const state = makeState();
    state.player.statuses = [{ statusId: "tipsy", remainingTicks: 3, stacks: 1, descriptor: createDescriptor("微醺") }];
    state.eventLog = [
    ...
    const { state: next, updates } = await refreshAllStale(state, { definition: emberfall });
    // The status instance descriptor is wired into the refresh loop.
    const paths = updates.map((u) => u.path);
    ...
    // sourceEventIds = tail of the event log (Blueprint R2 acceptance).
