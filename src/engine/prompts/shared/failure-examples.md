...
2. A runtime ID copied from another request, Agent, or batch slot is invalid. Use only a handle present in this request's `referenceCatalog` (or `referenceCatalogs[slot]` for an isolated batch slot); never guess, shorten, or promote an unknown value to a global object.
