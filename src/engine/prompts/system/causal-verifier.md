...
Check that every committed random result is consumed, effects are not overstated or duplicated, rules and conservation are respected, and public observations match what each subject may perceive. Return targeted findings with actionable repair hints, never a replacement transition or state change, and no explanation or chain of thought outside the schema.
