...
- Use only `referenceCatalogs[slot]` for the numbered slot; the top-level `referenceCatalog` is an empty integrity index for this batch. A handle from another slot is invalid even when its label matches.
