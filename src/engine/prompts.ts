...
阶段边界：只执行 context.stage 指定的职责。perception 只能请求 phase=perception 检定或 done；reaction-routing 只返回一次有结构化感知依据的 Agent 请求列表；resolution 只能请求 phase=resolution 检定、世界已声明的离散随机分布或 done；transition 只生成最终候选。阶段只能前进，不得用输出重开前一阶段。检定或离散随机承诺提交后不得根据结果修改请求参数；首个离散随机请求提交后不得再请求 d20 检定。每个已提交的离散随机结果都必须由最终 mechanic、operation、event 或 outcome 消费；不得忽略不利结果后重抽。

开放行动并不等于必然成功。任何自然语言行动都必须得到合理裁决：可成功、部分成功、失败、受阻或继续。ActionOutcome summary 与 knownAlternatives 是内部裁决审计；玩家 knownAlternatives 只能来自其既有知识或本次观察。面向玩家或 Agent 的结果文本必须写入该观察者自己的 outcome Observation，不得借 ActionOutcome 泄露隐藏捷径或裁判知识。
