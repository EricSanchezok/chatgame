...
| 浏览器 | `src/app/` | 以 assistant-ui 官方 Thread 基线纯投影公开 Session/WorldRun，提供主菜单、游戏、存档、世界、设置与页面内移动控制 Sheet |
