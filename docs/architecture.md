...
| 仿真内核 | `src/engine/` | 可替换执行算法生成候选；固定 CanonicalCommitter 验证因果、观察、认知与完整状态并构造唯一提交 |
| 模型网关 | `src/engine/model-*` | 模型目录、供应商适配、严格输出、公平队列与调用审计 |
| 执行证据 | `src/engine/observability.ts`、`src/server/execution-ledger.ts` | 用统一 execution/trace/artifact 协议记录正常运行、失败、Inspector、实验和重放 |
...
依赖方向固定为：浏览器 → Route Handler → WorldHost → SimulationEngine → WorldExecutionAlgorithm → TruthEngine/AgentMind；算法候选只能进入 CanonicalCommitter，不能直接修改 canonical state。`src/engine/` 与世界 YAML 不进入浏览器 bundle。
...
7. 全部结果有效后，revision、step、RNG、truth、belief、character、玩家知识和语义历史一次提交。canonical history 保存规则结果、随机承诺、断言、因果复核与 `semanticHash`；原子事务附加 execution ID、terminal sequence、trace hash 后计算 `contentHash`。模型请求、响应、配置和 invocation 明细只进入同一 SQLite Execution Ledger。任一失败则整个步骤回滚并保留失败 execution。
...
服务端以 execution/trace/span 串联 WorldRun、世界步骤、模型 transport 与 SQLite 持久化；失败和回滚与成功运行写入同一 Execution Ledger。事件、artifact 与指标边界见 [Execution Ledger](game-design/runtime-observability.md)。

世界演化调试器把 canonical history 通过事务校验共用的 replay 投影为 committed 图谱和 revision 前后快照，把 Execution Ledger 的 RuntimeEvent 投影为 attempt 分支。WorldHost 的 64 项 LRU 只缓存以 session、world hash 和 revision 寻址的可重建派生值；trace 与 live attempt 每次从 Ledger 查询。浏览器用 React Flow 呈现语义节点，Web Worker 内的 ELK 只计算坐标，不运行引擎或应用状态 delta。完整表面见 [表现层参考](game-design/presentation.md)。
...
浏览器不另存聊天消息；`PublicSessionDetail.runs` 按持久 `player.input` 边界投影成 assistant-ui 玩家/世界消息段和仅含公开叙事的复制文本。游戏页固定使用官方 `Root → Viewport → 44rem message group → ViewportFooter` 单轴结构，不显示 sidebar 或 session header；`/play/:sessionId` 持久布局拥有运行时与 SSE，存档和设置子路由只替换其上的模态管理层。新目标、clarification、重试、取消和放弃都回写 WorldRun 资源；所有成功或响应不确定的操作、跨标签页恢复和重新获得焦点都通过同一服务端对齐路径重建界面并只观察当前 executing run。
...
所有世界版本、当前版本指针、会话、run 与 Execution Ledger 存放在 `LIVINGWORLD_DATA_ROOT/livingworld.sqlite`。`WorldSessionDocument` schema v10 保存固定世界契约、1–80 字符存档标题、状态与 run；列表只返回公开摘要，读取与创建返回 `{ summary, state, runs }`。重命名与删除同样经过 generation fencing，queued/running 时均拒绝，避免元数据写入使在途步骤失效。世界包更新在写入前校验目标 ID，卸载在同一事务内拒绝关联存档并清除目录项和无引用版本。SQLite 使用 WAL、FULL synchronous、外键、严格表、写事务和 generation compare-and-swap；成功步骤的 session CAS 与 execution terminal record 在同一事务内提交。进程租约拒绝同一数据库被第二个宿主实例同时驱动；这是纯本地单实例契约，不提供多主或分布式协调。
...
当前状态模型没有地图格数量或动作种类上限；地点只是实体，移动只是带因果的 placement 变化。`monolithic-current@1` 故意激活并更新全部 Agent，以保存当前语义并暴露真实成本。稀疏激活、复用、局部依赖和局部重试必须作为新的算法 manifest 注册，并继续经过同一 CanonicalCommitter 与 Execution Ledger。

架构理由见 [0031](decisions/0031-epistemic-multi-agent-truth-engine.md)、[0032](decisions/0032-open-world-facts-and-d20-kernel.md)、[0033](decisions/0033-persistent-streaming-world-runs.md)、[0037](decisions/0037-agent-evolution-self-awareness-and-reaction-window.md)、[0039](decisions/0039-pinned-world-runtime-contract.md)、[0040](decisions/0040-resumable-player-intent.md)、[0041](decisions/0041-local-sqlite-runtime.md)、[0042](decisions/0042-causal-assurance-and-staged-model-profiles.md)、[0046](decisions/0046-committed-discrete-random-distributions.md)、[0047](decisions/0047-on-demand-model-provider-credentials.md)、[0048](decisions/0048-engine-owned-runtime-identities.md)、[0049](decisions/0049-world-run-failure-and-stream-boundaries.md)、[0055](decisions/0055-trusted-world-evolution-inspector.md) 与 [0059](decisions/0059-unified-execution-kernel-and-ledger.md)。
