...
| HTTP and browser | `src/app/` | API v12, world library, assistant-ui sessions, decision/reaction controls, unified Agent Perspective HUD, control orb, and read-only Inspector v9 with local Debug routes |
...
- Algorithms can emit only declared diagnostics; runtime event schema v3, stable lifecycle events, metric dimensions, and aggregation semantics remain engine-owned.
