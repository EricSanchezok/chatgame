...
- **`engine/index.ts`**（服务端）：默认导出 `(ctx: EngineExtensionContext) => void`，注册自定义效果 kind / 条件 source / 动作 handler / 规则机制 / 生命周期；动作 handler 先基于隔离的深度只读状态/定义快照返回纯 `ActionHandlerPlan`（拒绝、动态成本、耗时、单次 `execute`），引擎统一校验并支付动态成本，执行阶段才调用 `execute`。自定义持久状态写入 `WorldState.runtimeState`（引擎不解释，随完整严格校验的存档 v5 持久化）。由 `src/script/runtime-code.ts` 用 esbuild 编译为 CJS 加载（`.chatgame/build/<id>/` 内容 hash 缓存）。
