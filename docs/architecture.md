...
- **`engine/index.ts`**（服务端）：默认导出 `(ctx: EngineExtensionContext) => void`，注册自定义效果 kind / 条件 source / 动作 handler；handler 为纯函数（immutable state 更新），自定义持久状态写入 `WorldState.runtimeState`（引擎不解释，随存档 v4 持久化）。由 `src/script/runtime-code.ts` 用 esbuild 编译为 CJS 加载（`.chatgame/build/<id>/` 内容 hash 缓存）。
...
内置闭集（effect/condition/action 的白名单语义）保持不变；未知 kind/source/handler 在剧本校验时报错，运行期未注册则安全降级（效果跳过并记录、条件恒 false、动作未知拒绝）。
