...
1. **权威动作入口**：客户端与服务端共享 `TurnInput`、`IntentHint` 和 `ActionPreview` DTO。`intentHint` 只提供动作、目标和参数提示，引擎仍重新执行动作存在性、条件、冷却、成本、世界规则和处理器校验。动作 handler 返回纯 `ActionHandlerPlan`：计划包含拒绝、动态货币/物品/资源成本和权威耗时，`execute(state, grade)` 只在真实结算中调用一次；规划收到与权威 `WorldState`、`WorldDefinition`、参数完全隔离的深度只读 Proxy，execute 同样只收到 post-effect state 的深度只读 Proxy，任何层级的写入即使在非 strict CJS 中也响亮失败。execute 输出以调用前捕获的 `scriptId` 校验，并整体深度脱离代理后才进入后续 progression 与日志阶段；预检禁止通过 dry-run effects 或 execute 推测结果。引擎合并声明成本与动态成本，在骰点和效果前统一校验并仅扣除一次；预检用同一计划与可支付性检查。声明 `costs.time` 与 handler `timeCost` 都必须是非负有限整数小时，小数、负数、NaN 和 Infinity 响亮失败，handler 最终耗时在预检与执行都钳制为至少一小时。
2. **静态声明与精确注册**：带 `engine/index.ts` 的剧本必须在 `script.yaml.engine_extension` 声明 `api_version: 2` 及 effects、conditions、action handlers、rule mechanisms、lifecycle。validator 检查引用是否已声明，加载器检查声明集合与实际注册集合完全相等；代码与声明必须同时存在，声明和注册都不允许重复。未知 effect/condition/rule 在运行时同样响亮失败，不能静默跳过。custom condition 的 state/leaf/context 与 custom effect 的 state/effect/context 使用同一隔离的深度只读 Proxy；effect 输出以调用前捕获的 `scriptId` 校验并整体深度脱离代理后才成为权威状态。编译产物按剧本 ID、API 版本和完整 `engine/` 源码树 hash 内容寻址，同 ID 的预览与已安装版本不共享可变 bundle 文件。
3. **纯生命周期**：扩展可按注册顺序执行 `onSessionStart`、`onTurnResolved`、`onHour`、`onDayBoundary`。handler 的 state、definition、previousState、turnInput 和 resolution 都是隔离的深度只读 Proxy；handler 只返回新状态和摘要。引擎以调用前捕获的权威 `scriptId` 校验输出，不能通过原地修改绕过脚本边界，并把输出脱离只读代理后才交给下一阶段。session hook 仅在 fresh session 的开场前执行，load v5 不重放；turn hook 在动作与世界步进后执行；hour hook 在每小时衰减和日程更新后执行；day-boundary hook 在通用日界系统后执行。摘要进入可审计事件日志。
4. **规则机制闭集**：通用规则只接受 schema 导出的内置机制，剧本规则必须通过 v2 注册；诸如 `inventory`、`combat`、`travel` 的含糊空实现不是合法机制。剧本规则检查收到动作、任意实体目标及结构化参数的深度只读 Proxy，不能污染预检或执行的权威状态；预检与执行保持一致。
...
6. **持续系统与存档语义**：need 阈值采用持久化的边沿触发集合，持续低值不重复施加永久效果；所有 status 效果通过单一 `addStatus` 语义，重施加刷新时长，堆叠状态同时增加层数；成长只作用于触发行为对应的实体与 stat/skill。状态中新增长期事实要求存档 schema v5；序列化与反序列化共用完整、严格的 `SaveFile`/`WorldState` schema，envelope `scriptId` 必须等于 `worldState.scriptId` 且两者受载入目标约束。伪造版本号、脚本 ID 或必需/嵌套字段的 v5 必须拒绝，旧存档不提供兼容迁移。
