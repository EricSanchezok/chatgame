...
1. **权威动作入口**：客户端与服务端共享 `TurnInput`、`IntentHint` 和 `ActionPreview` DTO。`intentHint` 只提供动作、目标和参数提示，引擎仍重新执行动作存在性、条件、冷却、成本、世界规则和处理器校验。动作 handler 返回纯 `ActionHandlerPlan`：计划包含拒绝、动态货币/物品/资源成本和权威耗时，`execute(state, grade)` 只在真实结算中调用一次；预检只读计划，禁止通过 dry-run effects 或 execute 推测结果。预检合并声明成本与动态成本，旅行耗时来自实际选定路径。
...
5. **通用玩法语义**：任务目标是按类型区分的联合类型；investigate 明确声明 flag/fact marker 或 `any`，`any` 从激活日志后的事件游标开始计数，激活/完成/失败写入 `WorldState.eventLog`。`time_limit.days` 包含精确截止日，重复任务尊重完成/失败后的 cooldown。跨午夜日程按环形时间窗口判断。旅行按总分钟最短路径选择，并以逐段抵达时钟校验出口、连接和入口条件。声望效果在上升穿越阈值时立即、仅一次地触发阈值效果。秘密正文按运行时 `secretHolders` 注入与防泄露，定义 owner 不构成第二知识边界。
6. **持续系统语义**：need 阈值采用持久化的边沿触发集合，持续低值不重复施加永久效果；所有 status 效果通过单一 `addStatus` 语义，重施加刷新时长，堆叠状态同时增加层数；成长只作用于触发行为对应的实体与 stat/skill。状态中新增长期事实要求存档 schema v5，v5 缺少 `activeNeedThresholds` 也必须拒绝，旧存档不提供兼容迁移。
