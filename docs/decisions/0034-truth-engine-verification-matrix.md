...
Playwright 只对 `next build` 后的真实应用运行两类门禁：完整核心玩家流程和页面级 axe。测试从零世界启动，在隔离目录导入运行时生成的 schema v5 ZIP，经真实 Route Handler 创建会话、提交自由文本、接收 SSE、刷新并验证持久状态，同时检查窄屏无横向溢出。测试启动本地 HTTP 模型服务作为唯一替代边界，生产 `ModelGateway`、公平队列与 DeepSeek adapter 仍走真实代码。Storybook、浏览器组件模式、旧内置剧本 E2E 和像素基线不属于测试栈。
