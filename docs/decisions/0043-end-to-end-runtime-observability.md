...
`ModelExecutionAudit` 以 `invocations[]` 作为单一事实源，每个 invocation 保存请求/响应 hash 与字节、规范 Context 及顶层分区、对象计数、transport attempts、真实 usage、供应商结果与语义结论。汇总由 helper 派生。成功 bootstrap 和步骤持久化这些 invocation；失败步骤只保留运行日志。世界剧本保持 schema v5，`SimulationState` 使用 schema v7，`WorldSessionDocument` 使用 schema v8；旧运行态与会话直接拒绝且没有迁移路径。
...
- Schema v6 存档比聚合审计更大，但不重复保存汇总或原始内容。
