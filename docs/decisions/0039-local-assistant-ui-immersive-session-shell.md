...
对话层使用 MIT 许可的 `@assistant-ui/react` 原语和 `useExternalStoreRuntime`。每个持久 `WorldRun` 纯投影为一条玩家消息和一条世界消息；世界消息使用 data part 展示公开 observation、outcome、可见检定、替代方向与运行终态。前端不启用 Assistant Cloud、线程列表、附件、分支、编辑、重新生成或第二套消息存储。提交、取消和重试仍只调用 [0033](0033-persistent-streaming-world-runs.md) 定义的 WorldRun API，SSE 事件合并回唯一 `PublicSessionDetail`，消息数组始终由其派生。

Session 持久化文档使用 schema v4 并增加 1–80 字符标题；旧文档直接拒绝。公开 API 版本为 v2：列表返回含世界摘要、更新时间、步数和活动 run 的 `PublicSessionSummary`，单项读取与创建返回 `{ summary, state, runs }`，PATCH 修改标题，DELETE 在无 queued/running run 时永久删除。浏览器当前 Session 指针只是本机导航偏好，不是状态所有权或访问控制。
...
表现层采用宿主根部声明的 `--cg-*` token、叙事/界面/等宽三类字体、44px 最小目标、可见焦点、forced-colors 与减少动效。旧工作台组件、Tailwind/PostCSS 接线、旧选择首项逻辑和对应测试全部删除，不保留别名或隐藏回退。
...
- Session v4 断代会拒绝旧本地存档，符合快速迭代期不维护迁移层的约定。
