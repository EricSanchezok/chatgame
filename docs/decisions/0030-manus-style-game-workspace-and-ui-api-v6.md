...
会话与 composer 共享 52rem 中心轴。世界与 NPC 消息平铺，NPC 使用 24px 头像、姓名和职业身份行；玩家消息右对齐，最大 34rem，使用低对比填充。系统结果、判定与任务变化附着在对应世界回复内。地点和事件图片最大 45rem、16:9，并受视口高度约束；标题与说明属于同一紧凑 caption，点击图片打开居中 lightbox。

只有最新世界回复展示最多三个纵向建议行动。点击建议把文字写入 textarea、聚焦输入框并调用权威 `previewAction`；预检在 composer 内显示耗时、成本、风险或不可执行原因。输入文本发生任何编辑时，建议 hint、旧预检和竞态中的迟到结果立即失效。composer 是会话轴底部的粘性圆角表面，textarea 自动增长到 9rem 后内部滚动；Enter 发送、Shift+Enter 换行，并忽略中文 IME composing 的 Enter。界面只有一个 44px 发送按钮；离开底部时显示“跳到最新消息”。
