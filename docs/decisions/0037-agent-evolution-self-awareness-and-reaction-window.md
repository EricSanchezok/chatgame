...
世界剧本使用 schema v5，SimulationState 与 WorldSessionDocument 使用 schema v4。所有会进入状态字典或引用图的 ID 拒绝 JavaScript 原型保留键，避免模型输出、世界包或持久化文档污染对象原型。旧版本直接拒绝，不提供迁移或双轨兼容。
...
- [0039](0039-causal-assurance-and-staged-model-profiles.md) — 分阶段 Truth 与 Agent Profile、因果复核。
