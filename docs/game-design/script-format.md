...
- **ui/index.tsx**（前端扩展）：默认导出 `(ctx: ScriptUiContext) => void`，以 `ctx.configureGame(presentation)` 配置 UI API v6 的动态目标与建议行动，并以 `ctx.register(slot, { component })` 注册内容槽位；每个 bundle 最多配置一次 presentation，单例槽位不接受位置或排序参数，重复配置或重复注册使整个 bundle 激活失败。槽位、只读 props 与公共表现原语见 [presentation.md](presentation.md)；未配置或未注册时回退宿主默认表现。
