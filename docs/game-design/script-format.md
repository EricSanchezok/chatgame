...
- **ui/index.tsx**（前端扩展）：默认导出 `(ctx: ScriptUiContext) => void`，并以 `ctx.register(slot, { component })` 注册 UI API v3 槽位；单例槽位不接受位置或排序参数，重复注册使整个 bundle 激活失败。槽位与只读 props 见表现层规格 [presentation.md](presentation.md) 的 UI 拓扑；未注册槽位回退宿主默认组件。
