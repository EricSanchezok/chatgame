...
完整历史先验证 `commitmentRounds` 最多四轮、random 单轮最多十六个请求，并以原顺序恰好覆盖全部请求一次。每轮原因只可引用此前轮已提交的结果；同轮 self/sibling、跨步骤 check/random、resolution 后 perception、random 后 d20 都拒绝。perception check 使用 reaction 前 initial actions，resolution check/random 使用 final actions。随后从 `rngBefore` 先重放全部 d20，再按提交顺序重放离散随机请求，并逐字段核对 draws、aggregate、skipped 与 `rngAfter`。首次提交固定 definition initial state 的 canonical truth、player entity ID 与初始 Agent ID → entity ID 映射作为 replay base，其 hash 进入会话固定世界契约；恢复从该 base 按 operation 顺序重建 canonical truth 与动态 Agent 身份，而不把当前状态附带的任意 baseline 当作真相。每步再用与运行时相同的因果验证重新检查随机消费和全部断言，逐字段核对 target、assertion、passed、observed，并核对最终 canonical truth 和 actor identity；追加一个结果可重放、但没有被 transition 消费的承诺同样无法通过恢复。Truth transition、独立 causal verifier 与后续 semantic history 都收到同一轮界，因此能区分同轮并列承诺和查看前轮结果后的追加请求。会话校验还要求每个历史请求的 distribution snapshot 与会话固定世界契约中的同 ID 分布内容一致，因此相同 seed、前态、请求与轮界得到相同结果。归档导入对分布的规范化所有权见 [世界导入](script-import.md#原子安装)。
...
Observation summary 去除空白后不得为空。玩家 Observation 在提交前经过公开信息守卫；守卫从未公开私密事实、canonical identity 以及玩家尚不知道的其他 Agent 信念建立保护集合，命中保护内容的玩家 Observation 进入修复流程而不是被公开或提交。`ActionOutcome.summary` 与 `knownAlternatives` 保留在内部历史供裁决审计，不属于公共输出，因此不能作为玩家或 Agent 的叙事来源。世界 disclosure 是检定可见性的硬上限，模型只能保持或降低可见级别。
...
AgentMind v4 使用 model context contract v4，只获得自身人格、目标、belief、去 canonical 的局部绑定标记、自身可见的完整主观历史、自身上一行动、当前与历史 outcome status、本轮自己的 observations 和结构化修复问题。当前与历史 `perceivedOutcome` 都不携带 summary 或 alternatives；Agent 能看到的结果文本只来自自己的 Observation。它不获得 canonical truth、canonical ID、其他 Agent belief 或隐藏检定。OpenAI/xAI 由原生 strict JSON Schema 承载输出契约；DeepSeek 按官方 JSON mode 要求在 provider prompt 中附加精简 schema 和合法示例。完整调用协议见 [模型目录与 Gateway](model-gateway.md)。
