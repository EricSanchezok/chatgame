...
每个 `CommittedStep` 保存 base/new revision、联合行动、完整检定请求与结果、RNG 前后态、outcomes、事件、observations、delta operations、Agent belief patches 和模型审计。模型审计只含 role/subject/profile/provider/model、尝试与修复次数以及请求/响应 SHA-256；不保存 prompt、原始响应或思维链。整个步骤另有 canonical JSON 内容 hash。
