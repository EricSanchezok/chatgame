...
- Each provider's `api_key_env` names its credential environment variable. A credential is required only when the world or an Agent uses that provider's Profile. The bundled Blackmarsh reference world uses the DeepSeek profiles and therefore requires `DEEPSEEK_API_KEY` (unless its profiles are intentionally overridden for a test).
