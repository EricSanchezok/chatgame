...
- `LIVINGWORLD_DATA_ROOT` selects the local data directory and defaults to `.livingworld-v16/`. World versions, World Instances, WorldRuns, and the Execution Ledger share `livingworld.sqlite`. World Instance schema v16 embeds SimulationState v11 and its pinned algorithm reference; it does not read older saves, so use a new data root for another format.
...
Algorithm selection is an internal instance setting. The execution contract remains `bootstrap` and `step`; new algorithms register a versioned, JSON-safe manifest and factory, while stable telemetry and commit validation remain engine-owned. The built-in `eager-reference@3` is the explicit experiment default.
