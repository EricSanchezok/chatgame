...
Algorithm selection is an internal instance setting. Execution Contract v4 provides `bootstrap`, persistable Preparation v2 through `prepareStep`, and `completeStep`; new algorithms register a versioned, JSON-safe manifest and factory, while stable telemetry and commit validation remain engine-owned. The built-in `eager-reference@5` is the explicit experiment default.
