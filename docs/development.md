...
- `LIVINGWORLD_DATA_ROOT` selects the local data directory and defaults to `.livingworld-v17/`. World versions, World Instances, WorldRuns, reaction preparations, and the Execution Ledger share `livingworld.sqlite`. World Instance schema v17 embeds SimulationState v12 and its pinned algorithm reference; it does not read older saves, so use a new data root for another format.
...
Algorithm selection is an internal instance setting. The execution contract provides `bootstrap`, persistable `prepareStep`, and `completeStep`; new algorithms register a versioned, JSON-safe manifest and factory, while stable telemetry and commit validation remain engine-owned. The built-in `eager-reference@4` is the explicit experiment default.
