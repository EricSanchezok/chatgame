...
- `npm run test:live:deepseek:batching` is the credentialed eager-reference batching smoke. It executes real Blackmarsh Agent bootstrap and step preparation, asserts the default `12/8` limits, zero singleton failures, and invocation-ID uniqueness, and deliberately stops before the unchanged Truth-resolution boundary. `npm run test:live:deepseek` remains the broader end-to-end smoke.
...
- Action Compilation and AgentMind tests cover independent limits one, two, three, twelve, and sixty-four; stable tails; profile and purpose isolation; byte shrinking; exact slot coverage; localized repair; structural retry and recursive bisection; terminal error propagation; singleton rollback or fallback; and one unique audit per physical request. Deterministic singleton and larger batches must commit the same canonical semantic result.
