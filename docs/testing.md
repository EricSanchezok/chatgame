...
- `npm run experiment:run -- --agents 1,10,50,1000 --steps 1`：在统一 Ledger 中运行确定性规模矩阵；CI 执行小矩阵与 recorded replay 回归。
...
- 持久状态篡改测试不能停在重算 step/document 内容 hash；必须覆盖 `semanticHash`、跨步 prepared action、nextAction、belief/character、玩家首次目标与 clarification 输入链、历史 run 的 `intentId`/已提交输入前缀与同 revision canonical `playerIntent` 绑定、终态 intent 后的新 goal、Fact tombstone，以及公开 check/outcome/observation/commit 精确投影。模型 invocation ID、请求和响应完整性由 Execution Ledger 与 recorded replay 测试覆盖。
...
- Execution Ledger 测试必须证明完整 payload 可按 execution 取回、关键写入失败阻止 revision 前进、失败 execution 保留、原子 session/execution 提交、recorded replay 不触网且 hash 一致，并拒绝聚合指标中的高基数维度。
