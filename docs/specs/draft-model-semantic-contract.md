...
4. Use the Inspector API v9 audit trail to verify that a model call, its source invocation, and its related graph/step records share one normalized identity. The local Debug CLI can resolve that identity directly from a public invocation ID.
