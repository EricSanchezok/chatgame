...
Every Action Compilation batch receives one `referenceCatalog` whose candidates carry an opaque deterministic `candidateKey`, `kind`, `label`, `meaning`, `allowedUses`, `scope`, and optional `details`. The key is a request-local selector with no identity meaning; only the engine materializes it into a request-local handle after schema validation. Shared candidates are available to every slot and slot candidates are available only to their named slot. The projector includes details for references named by slot state, eligible temporal profiles, the world clock, candidates matched by action text, placement neighbors, and their reference closure; every other candidate remains present with `details: null`. Action Compilation does not also receive per-slot `referenceCatalogs`, `availableHandles`, raw `ref:*` values, or a second `canonicalTruth` representation of the same facts.
...
The historical C0-C5 comparison used one recorded workload. `C0` was the recorded request; `C1` shared the full catalog once; `C2` used one normalized complete catalog; `C3` retained all candidates while selecting deterministic details; `C4` added bounded evidence expansion; `C5` added retrieval. The experiment selected C3 lexicographically: semantic correctness and replay invariants, then commit/non-inferiority gates, then input and repair cost. The promoted production projection is the candidate-key protocol with deterministic details; comparison code and runtime flags were deleted, while the corpus and reports remain immutable evidence.
...
Maintain the checked-in recorded corpus, gold evaluator, and offline/live reports as the reproducibility surface. Production Action Compilation calls only `projectActionCompilationContextForModel`, emits the `candidate-key-v1-deterministic-details` projection, and materializes candidate keys through one resolver boundary. Historical comparison implementations and selection flags are not part of the codebase. Any replacement starts with a new correctness-first recorded and paired live experiment and updates this spec and [Decision 0088](../decisions/0088-action-compilation-candidate-key-protocol.md).
...
No candidate key may disappear between equivalent full and compact catalogs. The model-visible projection must contain zero serialized `ref:*` values and zero raw private references. Deterministic materialization must reject a temporal profile without its required evidence and must accept compatible exact spans. Repair must preserve valid slots, omit unrelated full context, and terminate repeated fingerprints without an identical model request. Recorded replay must preserve world/state/causal hashes and random commitments for accepted outputs.
