...
`executionTuning.truthBatchMaxSlots` is an integer from one through sixty-four and defaults to twelve. It is pinned in the eager-reference algorithm manifest, which is forward-only version `10`; old producers are obsolete saves and are not migrated.
