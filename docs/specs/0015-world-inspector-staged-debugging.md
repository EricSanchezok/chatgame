...
Implemented in the staged runtime, WorldHost control routes, Inspector projections, and replay controls. Completed-stage model outputs and audits are durable continuation evidence: after a database/process restart the engine deterministically rebuilds those stages without network access, then executes only the next stage. Recovery rejects missing evidence, source/runtime identity drift, invalid hashes, event ranges, or artifact references before canonical commit. Coverage includes logical-stage ordering and grouping, pre-commit debug pauses, CAS/idempotent next-step control, restart recovery, invalid preparation handling, semantic/technical graph projections, and replay frame derivation. The repository gates run the focused Vitest suites, typecheck, prompt/world validation, workflow verification, governance checks, and production build.
