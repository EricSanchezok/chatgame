...
The implementation ships the schema and loader contract, deterministic pool identity, one-pass grounding claims, queued/ready Activity union, fixed-point footprint index, atomic allocator, committer reconstruction, Host/public/Inspector projections, ordinary Observation rendering, and the forward-only API/runtime versions selected by [decision 0074](../decisions/0074-enforce-script-owned-shared-resource-pools.md). Deterministic unit and integration verification passes 259 tests in 40 files, including real Eager, CanonicalCommitter, replay, WorldHost, API, shared-resource and causal-regression entry paths; the schema fixture and Blackmarsh world with 232 Entities and 48 Agents both validate.

The production build passes, followed by all 11 browser flows and 4 accessibility flows. The schema v2 causal benchmark completes 240 scenarios with five samples each across 1/10/50/1000 Agents, four causal conflict densities, three resource contention densities, and five Activity types: affected recall, scenario, replay, and allocator/oracle agreement are 100%; false activation, causal violations, and model invocations are zero. The largest recorded queue is 500, interaction component 2000 nodes, artifact 2,089,902 bytes, footprint p95 93.89 ms, allocator p95 5.787 ms, and peak heap 150,075,040 bytes; timing and memory are evidence rather than CI thresholds. The causal interaction baseline remains [Spec 0005](0005-causal-activity-interactions.md).
